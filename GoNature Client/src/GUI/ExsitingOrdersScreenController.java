...

          {
            btn.setOnAction(
              (ActionEvent event) -> {
                ...
                ClientUI.orderController.order =
                  new Order(
                    orderNum,
                    lt,
                    ld,
                    data.getPark(),
                    Integer.parseInt(data.getNumOfVisit()),
                    Float.parseFloat(data.getPrice())
                  );
                ...
                try {
                  root =
                    loader.load(
                      getClass()
                        .getResource("/GUI/SureIfCancel.fxml")
                        .openStream()
                    );
                  Scene scene = new Scene(root);
            ...
            btn.setOnAction(
              (ActionEvent event) -> {
                ...
                if (
                  ...
                  try {
                    ClientUI.entranceParkController.setCurrentVisitros(
                      park,
                      numofvisit
                    );
                    if (
                      ClientUI.parkController.getCurrentVisitors(park) +
                      ClientUI.parkController.getCurrentUnexpectedVisitors(
                        park
                      ) ==
                      ClientUI.parkController.getMaxVisitors(park)
                    ) {
                      ClientUI.parkController.updateStatusForCapacityParkToFull(
                        park
                      );
                    }
                  } catch (IOException e) {
                  ...
                  Alert a = new Alert(
                    AlertType.NONE,
                    "Now it's not the time for your order!"
                  );
